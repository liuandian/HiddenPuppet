...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('log/ui_element_debug.log', encoding='utf-8'),
...
def find_ui_element(template_path, threshold=0.7):
    ...
    if max_val >= threshold:
        ...
        # 保存可视化结果
        output_path = "cache/match_result.png"
        ...
        # 保存原始截图以供调试
        output_path = "cache/failed_screenshot.png"
...
def create_mask(template, ignore_region=None):
    ...
    output_path = "cache/template_mask.png"
...
def non_max_suppression(locations, scores, template_w, template_h, iou_threshold=0.5):
    ...
    # 保存输入和输出到文件
    debug_path = "cache/nms_debug.txt"
...
def find_multi_ui_element(template_path, threshold=0.8, ignore_region=None):
    ...
    scores = result[result >= threshold]
    logger.debug(f"初始找到 {len(locations)} 个匹配位置")
    ...
    for loc in locations:
        top_left = loc
        center_x = top_left[0] + template_w // 2 + left
        center_y = top_left[1] + template_h // 2 + top
...
def find_miniprogram_element():
    template_path = "template/miniprogram.png"
    return  find_ui_element(template_path)

def find_forward_button_element():
    template_path = "template/forward_button.png"
    return  find_ui_element(template_path)

def find_search_group_button_element():
    template_path = "template/search_group.png"
    return  find_ui_element(template_path)

def find_send_msg_element():
    template_path = "template/send_msg.png"
    return  find_ui_element(template_path)

def find_final_send_element():
    template_path = "template/final_send.png"
    return  find_ui_element(template_path)

def find_multi_group_button_element():
    template_path = "template/multi_group.png"
...
if __name__ == "__main__":
    ...
    matches = find_multi_group_button_element()
    print(matches)