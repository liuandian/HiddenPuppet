...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('log/qq_window_debug.log', encoding='utf-8'),
...
def capture_qq_window(left, top, right, bottom, output_path="cache/qq_window_screenshot.png"):
    try:
...
def save_window_position(left, top, right, bottom, output_path="cache/window_position.json"):
    try:
...
def read_window_position(input_path="cache/window_position.json"):
    try:
    ...
    try:
        ...
        if hwnd:
            ...
            if screenshot is not None:
                print(f"截图已保存至: cache/qq_window_screenshot.png")
            else:
                print("截图失败，请检查日志")
        else:
            print("未找到 QQ 窗口")
